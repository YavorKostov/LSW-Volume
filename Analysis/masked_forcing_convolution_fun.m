function []= masked_forcing_convolution_fun(RegionName, MaskRegion, MemoryLength, NameSuffix, ForcingName, ForcingLength, ForcingDir, SensitivityFieldName, ScaleFactor, WinterSensitivityDir, SpringSensitivityDir, SummerSensitivityDir, FallSensitivityDir)

% Yavor Kostov

% Computes the contribution of a given source of forcing over a particular
% region (e.g., surface heat flux anomalies over the interior of the 
% Labrador Sea) to the variability in the objective function
...
% RegionName - name of the region we are considering, e.g., 'LabSea'. 
% This is used as a suffix in the output file name.

% MaskRegion - 2D mask of the region underconsideration (set to 1) 
% with zeros outside the region
...
% over the region of interest as a timeseries
...
% associated with the impact of forcing over a particular region.


global mygrid

FileNameSave=strcat(RegionName, '_cost_anom_', ForcingName, NameSuffix, '.mat');
...
for i=1:ForcingLength
    ...
    for j=1:MemoryLength
        ...
        if i+j-1<ForcingLength+1
            ...
            cost_anom_product=ForcingAnom.*SensitivityMap.*MaskRegion;
